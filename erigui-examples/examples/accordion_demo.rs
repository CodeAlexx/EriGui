    ...
    fn new() -> Self {
        ...
        let mut basic_accordion = Accordion::new(WidgetId::default())
            ...
            .with_on_toggle(|index, expanded| {
                println!("Panel {} {}", index, if expanded { "expanded" } else { "collapsed" });
            });
        ...
        single_accordion.add_panel(AccordionPanel::new(
            "Project Structure",
            Box::new(Label::new(WidgetId::default(), "src/\n  ├── main.rs\n  ├── lib.rs\n  └── modules/"))
        ).with_expanded(true).with_icon("folder"));
        ...
        single_accordion.add_panel(AccordionPanel::new(
            "Build Configuration",
            Box::new(Label::new(WidgetId::default(), "Target: x86_64-unknown-linux-gnu\nProfile: release\nOptimization: 3"))
        ).with_icon("file"));
        
        Self {
            basic_accordion,
            single_accordion,
            info_label: Label::new(WidgetId::default(), "Accordion Demo - Click headers to expand/collapse"),
        }
    }
}
...
fn main() {
    ...
    let mut renderer = Renderer::new(&event_loop, 800, 700, "Accordion Demo")
        ...
        match event {
            WinitEvent::WindowEvent { event, .. } => {
                ...
                if let Some(gui_event) = erigui_rendering::window::convert_window_event(event, renderer.viewport_size()) {
                    ...
                    if basic_result == EventResult::Consumed || single_result == EventResult::Consumed {
            ...
            WinitEvent::RedrawRequested(_) => {
                ...
                demo.basic_accordion.layout(Rect::new(
                    padding,
                    accordion_y,
                    accordion_width,
                    basic_size.height
                ...
                demo.single_accordion.layout(Rect::new(
                ...
                demo.basic_accordion.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.single_accordion.draw(&mut renderer as &mut dyn DrawContext, &theme);
