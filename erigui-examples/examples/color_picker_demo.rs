        ...
        *control_flow = ControlFlow::Wait;
        ...
        match event {
            ...
            WinitEvent::RedrawRequested(_) => {
                ...
                let compact_label_size = demo.compact_label.measure(&LayoutConstraints::default(), &theme);
                demo.compact_label.layout(Rect::new(padding, y, compact_label_size.width, compact_label_size.height), &theme);
                y += compact_label_size.height + 10;
                
                let compact_size = demo.compact_picker.measure(&LayoutConstraints::default(), &theme);
                demo.compact_picker.layout(Rect::new(padding, y, compact_size.width, compact_size.height), &theme);
                
                // Color preview box for compact picker
                let preview_size = 60;
                let compact_color = demo.compact_picker.get_color();
                let compact_preview_rect = Rect::new(padding + compact_size.width + 20, y - 10, preview_size, preview_size);
                
                y += compact_size.height + section_spacing;
                
                // No alpha picker section
                let no_alpha_label_size = demo.no_alpha_label.measure(&LayoutConstraints::default(), &theme);
                demo.no_alpha_label.layout(Rect::new(padding, y, no_alpha_label_size.width, no_alpha_label_size.height), &theme);
                y += no_alpha_label_size.height + 10;
                
                let no_alpha_size = demo.no_alpha_picker.measure(&LayoutConstraints::default(), &theme);
                demo.no_alpha_picker.layout(Rect::new(padding, y, no_alpha_size.width, no_alpha_size.height), &theme);
                
                // Color preview box for no alpha picker
                let no_alpha_color = demo.no_alpha_picker.get_color();
                let no_alpha_preview_rect = Rect::new(padding + no_alpha_size.width + 20, y - 10, preview_size, preview_size);
                
                y += no_alpha_size.height + section_spacing;
                
                // Square picker section (always visible)
                let square_label_size = demo.square_label.measure(&LayoutConstraints::default(), &theme);
                demo.square_label.layout(Rect::new(padding + 300, padding, square_label_size.width, square_label_size.height), &theme);
                
                let square_size = demo.square_picker.measure(&LayoutConstraints::default(), &theme);
                demo.square_picker.layout(Rect::new(padding + 300, padding + square_label_size.height + 10, square_size.width, square_size.height), &theme);
                
                // Draw
                renderer.begin_frame(theme.colors.background);
                
                // Draw labels and pickers
                demo.compact_label.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.compact_picker.draw(&mut renderer as &mut dyn DrawContext, &theme);
                
                demo.no_alpha_label.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.no_alpha_picker.draw(&mut renderer as &mut dyn DrawContext, &theme);
                
                demo.square_label.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.square_picker.draw(&mut renderer as &mut dyn DrawContext, &theme);
