...
impl DockPanelDemo {
    fn new() -> Self {
        ...
        let editor2_content = Box::new(Label::new(
            WidgetId::default(),
        ...
        Self {
            ...
            info_label: Label::new(
                WidgetId::default(),
                "DockPanel Demo - Drag panels to rearrange, drag splitters to resize",
            ),
        }
    }
}
...
fn main() {
    ...
    let mut renderer =
        Renderer::new(&event_loop, 1024, 768, "DockPanel Demo").expect("Failed to create renderer");

    let theme = Theme::dark();

    let mut demo = DockPanelDemo::new();

    // Persistent translator. The `convert_window_event` shortcut is
    // deprecated because it instantiates a fresh translator per call,
    // which loses last_cursor and reports every click as (0, 0).
    ...
    event_loop.run(move |event, elwt| {
        ...
        match event {
            ...
            WinitEvent::WindowEvent { event: WindowEvent::RedrawRequested, .. } => {
                ...
                demo.dock_panel.layout(
                    Rect::new(0, padding, window_size.width, window_size.height - padding),
                    &theme,
                );
