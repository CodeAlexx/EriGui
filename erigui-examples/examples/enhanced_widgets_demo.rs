...
impl EnhancedWidgetsDemo {
    fn new() -> Self {
        ...
        let mut drag_source_list = ListView::new(WidgetId::default())
            .with_items(vec![
                ListItem::new("Item 1", "item1"),
                ListItem::new("Item 2", "item2"),
                ListItem::new("Item 3", "item3"),
                ListItem::new("Item 4", "item4"),
                ListItem::new("Item 5", "item5"),
            ]);
        
        let drop_target_list = ListView::new(WidgetId::default())
            .with_items(vec![
                ListItem::new("Drop items here", "placeholder"),
            ]);
        ...
        let nav_text_input = TextInput::new(WidgetId::default())
            .with_placeholder("Type here...");
        
        let nav_combo = ComboBox::new(WidgetId::default())
            .with_items(vec![
                "Option 1".to_string(),
                "Option 2".to_string(),
                "Option 3".to_string(),
        ...
        let announcements_text = TextArea::new(WidgetId::default())
            .with_placeholder("Screen reader announcements will appear here...");
        ...
        Self {
            drag_source_list,
            drop_target_list,
            drag_label: Label::new(WidgetId::default(), "Drag items from left list to right list"),
            nav_buttons,
            nav_text_input,
            nav_combo,
            nav_checkbox,
        ...
        *control_flow = ControlFlow::Wait;
        ...
        match event {
            WinitEvent::WindowEvent { event, .. } => {
                ...
                if let Some(gui_event) = erigui_rendering::window::convert_window_event(event, renderer.viewport_size()) {
                    ...
                    if let Event::KeyPress(key_event) = &gui_event {
                        let mut nav_manager = keyboard_nav_manager();
                        if let Some(next_widget) = nav_manager.handle_key(key_event.key) {
                            ...
                            demo.nav_text_input.set_focused(demo.nav_text_input.id() == next_widget);
                            demo.nav_combo.set_focused(demo.nav_combo.id() == next_widget);
                            demo.nav_checkbox.set_focused(demo.nav_checkbox.id() == next_widget);
                        }
                    }
                    ...
                    // Drag source list
                    if demo.drag_source_list.handle_event(&gui_event, &theme) == EventResult::Consumed {
                    ...
                    // Drop target list
                    if demo.drop_target_list.handle_event(&gui_event, &theme) == EventResult::Consumed {
                        needs_redraw = true;
                    }
                    ...
                    if let Event::MouseButton(mouse_event) = &gui_event {
                        if !mouse_event.pressed && drag_manager.is_dragging() {
                            if drag_manager.check_drop_target(demo.drop_target_list.id(), demo.drop_target_list.bounds()) {
                                if let Some(drag_data) = drag_manager.complete_drop() {
                                    if let Ok(item) = drag_data.data.downcast::<ListItem>() {
                                        demo.drop_target_list.add_item(*item);
                                        
                                        // Announce the drop
                                        let mut a11y_manager = accessibility_manager();
                                        a11y_manager.announce(format!("Item dropped into target list"));
                                    }
                                }
                            } else {
                    ...
                    for (i, button) in demo.nav_buttons.iter_mut().enumerate() {
                        if button.handle_event(&gui_event, &theme) == EventResult::Consumed {
                            if let Event::MouseButton(e) = &gui_event {
                                if !e.pressed {
                                    let mut a11y_manager = accessibility_manager();
                                    a11y_manager.perform_action(button.id(), AccessibilityAction::Click);
                                }
                            }
                            needs_redraw = true;
                        }
                    ...
                    if demo.nav_checkbox.handle_event(&gui_event, &theme) == EventResult::Consumed {
                        let mut a11y_manager = accessibility_manager();
                        a11y_manager.update_node_state(demo.nav_checkbox.id(), AccessibilityState {
                    ...
                    if demo.announcements_text.handle_event(&gui_event, &theme) == EventResult::Consumed {
                        needs_redraw = true;
                    }
            ...
            WinitEvent::RedrawRequested(_) => {
                ...
                demo.drag_label.layout(Rect::new(
                    padding,
                    drag_section_y,
                ...
                demo.drag_source_list.layout(Rect::new(
                    padding,
                    drag_section_y + 40,
                    list_width,
                    list_height
                ...
                demo.drop_target_list.layout(Rect::new(
                ...
                let nav_label = Label::new(WidgetId::default(), "Keyboard Navigation (Use Tab/Arrow keys)");
                let mut nav_label_mut = nav_label;
                nav_label_mut.layout(Rect::new(
                    padding,
                    nav_section_y,
                    window_size.width - padding * 2,
                    30
                ...
                demo.nav_text_input.layout(Rect::new(x, y, 200, button_height), &theme);
                nav_manager.update_widget_bounds(demo.nav_text_input.id(), demo.nav_text_input.bounds());
                x += 200 + widget_spacing;
                
                demo.nav_combo.layout(Rect::new(x, y, 150, button_height), &theme);
                nav_manager.update_widget_bounds(demo.nav_combo.id(), demo.nav_combo.bounds());
                
                demo.nav_checkbox.layout(Rect::new(padding, y + button_height + 10, 200, 30), &theme);
                nav_manager.update_widget_bounds(demo.nav_checkbox.id(), demo.nav_checkbox.bounds());
                
                // Accessibility section
                let a11y_section_y = nav_section_y + section_height + padding;
                
                demo.accessibility_label.layout(Rect::new(
                    padding,
                    a11y_section_y,
                    window_size.width - padding * 2,
                    30
                ...
                demo.announcements_text.layout(Rect::new(
                    padding,
                    a11y_section_y + 40,
                    window_size.width - padding * 2,
                    section_height - 50
                ...
                // Draw widgets
                demo.drag_label.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.drag_source_list.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.drop_target_list.draw(&mut renderer as &mut dyn DrawContext, &theme);
                ...
                demo.nav_text_input.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.nav_combo.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.nav_checkbox.draw(&mut renderer as &mut dyn DrawContext, &theme);
                
                demo.accessibility_label.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.announcements_text.draw(&mut renderer as &mut dyn DrawContext, &theme);
