...
impl RadioButtonDemo {
    ...
    fn update_labels(&mut self) {
        ...
        self.size_label.set_text(format!("Selected size: {}", self.selected_size));
        self.color_label.set_text(format!("Selected color: {}", self.selected_color));
    }
}
        ...
        *control_flow = ControlFlow::Wait;
        ...
        match event {
            ...
            WinitEvent::RedrawRequested(_) => {
                ...
                let mut y = padding;
                let size_label_size = demo.size_label.measure(&LayoutConstraints::default(), &theme);
                demo.size_label.layout(Rect::new(padding, y, size_label_size.width, size_label_size.height), &theme);
                y += size_label_size.height + 10;
                
                let size_small_size = demo.size_small.measure(&LayoutConstraints::default(), &theme);
                demo.size_small.layout(Rect::new(padding, y, size_small_size.width, size_small_size.height), &theme);
                y += item_spacing;
                
                let size_medium_size = demo.size_medium.measure(&LayoutConstraints::default(), &theme);
                demo.size_medium.layout(Rect::new(padding, y, size_medium_size.width, size_medium_size.height), &theme);
                y += item_spacing;
                
                let size_large_size = demo.size_large.measure(&LayoutConstraints::default(), &theme);
                demo.size_large.layout(Rect::new(padding, y, size_large_size.width, size_large_size.height), &theme);
                y += section_spacing;
                
                // Color selection section
                let color_label_size = demo.color_label.measure(&LayoutConstraints::default(), &theme);
                demo.color_label.layout(Rect::new(padding, y, color_label_size.width, color_label_size.height), &theme);
                y += color_label_size.height + 10;
                
                let color_red_size = demo.color_red.measure(&LayoutConstraints::default(), &theme);
                demo.color_red.layout(Rect::new(padding, y, color_red_size.width, color_red_size.height), &theme);
                y += item_spacing;
                
                let color_green_size = demo.color_green.measure(&LayoutConstraints::default(), &theme);
                demo.color_green.layout(Rect::new(padding, y, color_green_size.width, color_green_size.height), &theme);
                y += item_spacing;
                
                let color_blue_size = demo.color_blue.measure(&LayoutConstraints::default(), &theme);
                demo.color_blue.layout(Rect::new(padding, y, color_blue_size.width, color_blue_size.height), &theme);
                y += section_spacing;
                
                // Label position demonstration
                let position_label_size = demo.position_label.measure(&LayoutConstraints::default(), &theme);
                demo.position_label.layout(Rect::new(padding, y, position_label_size.width, position_label_size.height), &theme);
                ...
                demo.pos_left.layout(Rect::new(padding, y, pos_left_size.width + 20, pos_left_size.height), &theme);
                
                let pos_right_size = demo.pos_right.measure(&LayoutConstraints::default(), &theme);
                demo.pos_right.layout(Rect::new(padding + 200, y, pos_right_size.width, pos_right_size.height), &theme);
                
                y += 50;
                
                let pos_top_size = demo.pos_top.measure(&LayoutConstraints::default(), &theme);
                demo.pos_top.layout(Rect::new(padding, y, pos_top_size.width, pos_top_size.height + 20), &theme);
                
                let pos_bottom_size = demo.pos_bottom.measure(&LayoutConstraints::default(), &theme);
                demo.pos_bottom.layout(Rect::new(padding + 200, y, pos_bottom_size.width, pos_bottom_size.height + 20), &theme);
                
                // Draw
                renderer.begin_frame(theme.colors.background);
                
                demo.size_label.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.size_small.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.size_medium.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.size_large.draw(&mut renderer as &mut dyn DrawContext, &theme);
                
                demo.color_label.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.color_red.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.color_green.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.color_blue.draw(&mut renderer as &mut dyn DrawContext, &theme);
                
                demo.position_label.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.pos_left.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.pos_right.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.pos_top.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.pos_bottom.draw(&mut renderer as &mut dyn DrawContext, &theme);
