...
Mouse selection and keyboard selection are both supported."#;
        ...
        let text_area = TextArea::new(WidgetId::default())
            ...
            .with_on_change(|text| {
                println!("Text changed, {} lines, {} chars", 
                    text.lines().count(),
                    text.len()
                );
            });
        ...
        Self {
            text_area,
            info_label: Label::new(WidgetId::default(), "TextArea Demo - Multi-line text editor"),
            line_numbers_checkbox: Checkbox::new(WidgetId::default(), "Show line numbers")
                .with_checked(true),
            word_wrap_checkbox: Checkbox::new(WidgetId::default(), "Word wrap")
                .with_checked(false),
        }
    }
    ...
    fn update_settings(&mut self) {
        ...
        let chars = text.len();
        let selected = self.text_area.get_selected_text()
            .map(|s| s.len())
            .unwrap_or(0);
        
        let info = if selected > 0 {
            format!("Lines: {} | Characters: {} | Selected: {}", lines, chars, selected)
        } else {
...
fn main() {
    ...
    let mut renderer = Renderer::new(&event_loop, 900, 700, "TextArea Demo")
        ...
        *control_flow = ControlFlow::Wait;
        ...
        match event {
            ...
            WinitEvent::RedrawRequested(_) => {
                ...
                // Controls at top
                let checkbox_size = demo.line_numbers_checkbox.measure(&LayoutConstraints::default(), &theme);
                demo.line_numbers_checkbox.layout(Rect::new(
                    padding,
                    padding,
                    checkbox_size.width,
                ...
                let wrap_checkbox_size = demo.word_wrap_checkbox.measure(&LayoutConstraints::default(), &theme);
                demo.word_wrap_checkbox.layout(Rect::new(
                ...
                demo.text_area.layout(Rect::new(
                    padding,
                    text_area_y,
                ...
                demo.line_numbers_checkbox.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.word_wrap_checkbox.draw(&mut renderer as &mut dyn DrawContext, &theme);
                demo.text_area.draw(&mut renderer as &mut dyn DrawContext, &theme);
