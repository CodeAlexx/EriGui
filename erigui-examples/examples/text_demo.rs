use erigui_core::{
    Color, DrawContext, LayoutConfig, LayoutMode, Margins, Point, Rect, Size, Theme, Widget,
};
...
impl App {
    fn new() -> Self {
        ...
        let title_id = widget_manager.add_widget(Box::new(
            Label::new(WidgetId::default(), "Text Rendering Demo").with_align(TextAlign::Center),
        ));
        ...
        let subtitle_id = widget_manager.add_widget(Box::new(
            Label::new(
                WidgetId::default(),
                "Testing various text features in EriGui",
            )
        ...
        let long_text_id = widget_manager.add_widget(Box::new(Label::new(
            WidgetId::default(),
        ...
        let numbers_text_id = widget_manager.add_widget(Box::new(Label::new(
            WidgetId::default(),
            "Numbers: 0123456789 Mix: abc123XYZ!@#",
        )));
        ...
        let button2_id = widget_manager.add_widget(Box::new(Button::new(
            WidgetId::default(),
            "Medium Length Button Text",
        )));

        let button3_id = widget_manager.add_widget(Box::new(Button::new(
            WidgetId::default(),
            "Very Long Button Text That Might Overflow",
        )));

        // Disabled text
        let disabled_label_id = widget_manager.add_widget(Box::new(Label::new(
            WidgetId::default(),
            "This text is disabled",
        )));
        widget_manager
            .get_mut(disabled_label_id)
            .unwrap()
            .set_enabled(false);
        ...
        let button_container = widget_manager
            .get_typed_mut::<Container>(button_container_id)
            .unwrap();
