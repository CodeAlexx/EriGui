...
impl TooltipDemo {
    fn new() -> Self {
        ...
        tooltip_manager().register_tooltip(
            ...
            Tooltip::new(
                checkbox.id(),
                "Check this box to enable the special feature",
            )
            .with_delay(300),
        );

        let slider = Slider::new(WidgetId::default(), 0.0, 100.0, 50.0);
        tooltip_manager().register_tooltip(
            slider.id(),
            Tooltip::new(slider.id(), "Drag to adjust the value").with_delay(800),
        );
        ...
        Self {
            ...
            title_label: Label::new(
                WidgetId::default(),
                "Tooltip Demo - Hover over widgets to see tooltips",
            ),
                    ...
                    erigui_rendering::window::convert_window_event(event, renderer.viewport_size())
                {
                    ...
                    if let Event::MouseMove(move_event) = &gui_event {
                        ...
                        if let Some((widget_id, _)) = hover_info {
                            demo.hover_label
                                .set_text(format!("Hovering over widget: {:?}", widget_id));
                        } else {
