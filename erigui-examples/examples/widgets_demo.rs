use erigui_core::{Color, Rect, Size, Point, Margins, LayoutConfig, LayoutMode, Widget, Theme};
use erigui_rendering::{Renderer, convert_window_event};
use erigui_widgets::{
    Container, Label, Checkbox, Slider, ProgressBar, TextAlign,
    WidgetManager, WidgetId, SliderOrientation, ProgressBarStyle
};
...
impl App {
    fn new() -> Self {
        ...
        let title_id = widget_manager.add_widget(Box::new(
            Label::new(WidgetId::default(), "EriGui Widgets Demo")
    ...
    fn create_checkbox_section(widget_manager: &mut WidgetManager) -> WidgetId {
        ...
        let checkbox3_id = widget_manager.add_widget(Box::new(
            Checkbox::new(WidgetId::default(), "Disabled option")
        ));
        widget_manager.get_mut(checkbox3_id).unwrap().set_enabled(false);
    ...
    fn create_slider_section(widget_manager: &mut WidgetManager) -> WidgetId {
        ...
        // Value label
        let value_label_id = widget_manager.add_widget(Box::new(
            Label::new(WidgetId::default(), "Volume: 50%")
        ));
        ...
        let h_slider_id = {
            ...
            widget_manager.add_widget(Box::new(
                Slider::new(WidgetId::default(), 0.0, 100.0, 50.0)
                    .with_on_value_changed(move |value| {
        ...
        let brightness_label_id = widget_manager.add_widget(Box::new(
            Label::new(WidgetId::default(), "Brightness:")
        ));
        ...
        // Add to containers
        let v_container = widget_manager.get_typed_mut::<Container>(v_container_id).unwrap();
    ...
    fn create_progress_section(widget_manager: &mut WidgetManager) -> (WidgetId, WidgetId, WidgetId) {
        let container_id = widget_manager.add_widget(Box::new(
            Container::new(WidgetId::default()).with_layout(LayoutConfig {
                mode: LayoutMode::Vertical,
                spacing: 10,
    ...
    fn update(&mut self) {
        ...
        if let Some(progress) = self.widget_manager.get_typed_mut::<ProgressBar>(self.animated_progress_id) {
            progress.animate();
        ...
        if let Some(progress) = self.widget_manager.get_typed_mut::<ProgressBar>(self.progress_bar_id) {
            progress.animate();
        }
    }
        ...
        match layout.mode {
            LayoutMode::Vertical => {
                ...
                for &child_id in children {
                    ...
                    let is_vertical_slider = self.widget_manager.get_typed::<Slider>(child_id)
