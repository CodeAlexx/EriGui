...
impl Breadcrumb {
    ...
    pub fn navigate_to(&mut self, index: usize) {
        if self.home_item.is_some() && index == 0 {
            ...
            let actual_index = if self.home_item.is_some() { index - 1 } else { index };
    ...
    fn draw_item(&self, context: &mut dyn DrawContext, theme: &Theme, text: &str, rect: Rect, is_hovered: bool, is_pressed: bool, enabled: bool) {
    ...
    fn draw_separator(&self, context: &mut dyn DrawContext, theme: &Theme, position: Point) {
        ...
        context.draw_text(
            &self.separator,
            Point::new(position.x, position.y + theme.typography.font_size_base / 2 - 2),
            theme.typography.font_size_base
        );
    }
}
        ...
        if let Some(home) = &self.home_item {
            if item_index < self.item_rects.len() {
                ...
                if self.show_home_icon {
                    ...
                    self.draw_item(context, theme, &home.text, rect, is_hovered, is_pressed, home.enabled);
                }
        ...
        for (i, item) in self.items.iter().enumerate() {
            ...
            if i > 0 || self.home_item.is_some() {
                ...
                self.draw_separator(
                    context,
                    theme,
                    Point::new(sep_x, self.state.bounds.center().y - theme.typography.font_size_base / 2 + 2)
                );
            }
            ...
            if item_index < self.item_rects.len() {
                ...
                self.draw_item(context, theme, &item.text, rect, is_hovered, is_pressed, item.enabled);
