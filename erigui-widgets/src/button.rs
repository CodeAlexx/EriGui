...
impl Widget for Button {
    ...
    fn draw(&self, context: &mut dyn DrawContext, theme: &Theme) {
        ...
        let (bg_color, text_color, border_color) = if !self.state.enabled {
            (
        ...
        let _content_height = theme.typography.font_size_base.max(if self.icon.is_some() { 20 } else { 0 });
        ...
        if !self.text.is_empty() {
            ...
            let text_pos = Point::new(
                x,
                bounds.center().y - text_size.height / 2,
            );
