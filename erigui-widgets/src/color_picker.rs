use erigui_core::{
    ...
    MouseButtonEvent, MouseMoveEvent, Point, Rect, Size, Theme, Widget, WidgetId, WidgetState,
};
use std::any::Any;
    ...
    fn draw_popup(&self, context: &mut dyn DrawContext, theme: &Theme) {
        ...
        for py in 0..sv_size {
            for px in 0..sv_size {
                ...
                context.fill_rect(Rect::new(sv_rect.x() + px, sv_rect.y() + py, 1, 1));
            }
        }
        ...
        if self.show_alpha {
            ...
            // Draw alpha cursor
            let alpha_cursor_x =
                alpha_rect.x() + (self.color.a as f32 / 255.0 * sv_size as f32) as i32;
            context.set_color(Color::WHITE);
            context.fill_rect(Rect::new(
                alpha_cursor_x - 2,
                alpha_rect.y() - 2,
        ...
        context.set_color(if self.hex_focused {
        ...
        context.draw_text(
            "#",
            Point::new(hex_rect.x() + 4, hex_rect.center().y + 6),
        ...
        context.draw_text(
            &self.hex_input,
            Point::new(hex_rect.x() + 16, hex_rect.center().y + 6),
            theme.typography.font_size_base,
        );
                ...
                if self.popup_visible {
                    ...
                    if popup_rect.contains(*position) {
                        ...
                        if hue_rect.contains(*position)
                            || (position.y >= hue_rect.y() - 5
                                && position.y <= hue_rect.bottom() + 5
                                && position.x >= hue_rect.x()
                                && position.x <= hue_rect.right())
                        {
                            if *pressed {
                        ...
                        if self.show_alpha {
                            let alpha_rect = Rect::new(content_x, y, sv_size, hue_height);
                            if alpha_rect.contains(*position)
                                || (position.y >= alpha_rect.y() - 5
                                    && position.y <= alpha_rect.bottom() + 5
                                    && position.x >= alpha_rect.x()
                                    && position.x <= alpha_rect.right())
                            {
                                if *pressed {
                                    self.dragging_alpha = true;
                                    let alpha =
                                        ((position.x - alpha_rect.x()) as f32 / sv_size as f32
                                            * 255.0) as u8;
                                    self.color.a = alpha;
                                    if let Some(callback) = &mut self.on_change {
                                        callback(self.color);
                ...
                if self.popup_visible {
                    ...
                    if self.dragging_hue {
                        let hue_rect = Rect::new(
                            content_x,
                            popup_rect.y() + padding + sv_size + padding,
                            sv_size,
                            20,
                        );
                    ...
                    if self.dragging_alpha && self.show_alpha {
                        let alpha_rect = Rect::new(
                            content_x,
                            popup_rect.y() + padding + sv_size + padding + 20 + padding,
                            sv_size,
                            20,
                        );
                        let alpha =
                            ((position.x - alpha_rect.x()) as f32 / sv_size as f32 * 255.0) as u8;
                        self.color.a = alpha;
                        if let Some(callback) = &mut self.on_change {
                            callback(self.color);
                        }
