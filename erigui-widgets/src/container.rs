use erigui_core::{
    DrawContext, Event, EventResult, FlexChild, LayoutConfig, LayoutConstraints, LayoutMode, Rect,
...
impl Container {
    ...
    fn layout_children<'a>(
        &self,
        widgets: &'a mut dyn FnMut(WidgetId) -> Option<&'a mut dyn Widget>,
        theme: &Theme,
    ) {
                ...
                for (i, &child_id) in self.children.iter().enumerate() {
                    if self.flex_children[i].flex == 0.0 {
                        if let Some(child) = widgets(child_id) {
                            let constraints =
                                LayoutConstraints::bounded(content_rect.width(), i32::MAX);
            ...
            LayoutMode::Horizontal => {
                ...
                for (i, &child_id) in self.children.iter().enumerate() {
                    if self.flex_children[i].flex == 0.0 {
                        if let Some(child) = widgets(child_id) {
                            let constraints =
                                LayoutConstraints::bounded(i32::MAX, content_rect.height());
                            let size = child.measure(&constraints, theme);
