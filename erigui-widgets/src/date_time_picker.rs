use erigui_core::{
    ...
    Point, Rect, Size, Theme, Widget, WidgetId, WidgetState, Key, KeyPressEvent,
};
use std::any::Any;
use chrono::{Local, Datelike, Timelike, NaiveDate, NaiveTime, NaiveDateTime};
use crate::{TextInput, Button, Icon};
...
impl DateTimePicker {
    ...
    fn update_input_text(&mut self) {
        let text = match self.mode {
            ...
            DateTimePickerMode::DateTime => format!("{} {}", self.format_date(), self.format_time()),
        };
    ...
    fn draw_calendar(&self, context: &mut dyn DrawContext, theme: &Theme) {
        ...
        let header_text = format!("{} {}", month_names[(self.viewing_month - 1) as usize], self.viewing_year);
        context.draw_text(
            &header_text,
            Point::new(
        ...
        for day in 1..=days_in_month {
            ...
            context.draw_text(&day_text, Point::new(text_x, text_y), theme.typography.font_size_base);
        }
    }
    ...
    fn draw_time_picker(&self, context: &mut dyn DrawContext, theme: &Theme) {
        ...
        if self.time_format == TimeFormat::H12 {
            ...
            context.draw_text("AM", Point::new(am_rect.center().x - 8, am_rect.center().y + 4), theme.typography.font_size_base);
            ...
            context.draw_text("PM", Point::new(pm_rect.center().x - 8, pm_rect.center().y + 4), theme.typography.font_size_base);
        }
    }
    ...
    fn handle_calendar_click(&mut self, position: Point) -> bool {
        ...
        for day in 1..=days_in_month {
            ...
            if day_rect.contains(position) {
                ...
                let new_date = NaiveDate::from_ymd_opt(self.viewing_year, self.viewing_month, day).unwrap();
                self.selected_datetime = NaiveDateTime::new(new_date, self.selected_datetime.time());
                self.update_input_text();
                self.notify_change();
    ...
    fn update_time_from_inputs(&mut self) {
        ...
        let final_hour = match self.time_format {
            ...
            TimeFormat::H12 => {
                ...
                if h == 12 {
                    if self.am_pm_selected { 12 } else { 0 }
                } else {
                    if self.am_pm_selected { h + 12 } else { h }
                }
            }
        };
...
impl Widget for DateTimePicker {
    ...
    fn layout(&mut self, rect: Rect, theme: &Theme) {
        ...
        // Layout button
        self.calendar_button.layout(Rect::new(
        ...
        let time_picker_height = if self.time_format == TimeFormat::H12 { 120 } else { 80 };
