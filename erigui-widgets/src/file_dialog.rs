use erigui_core::{
    ...
    Rect, Size, Theme, Widget, WidgetId, WidgetState, Key, KeyPressEvent,
};
use std::any::Any;
use std::path::{Path, PathBuf};
use std::fs;
use crate::{Button, TextInput, ListView, ListItem, Breadcrumb, BreadcrumbItem};
...
impl Widget for FileDialog {
    ...
    fn layout(&mut self, rect: Rect, theme: &Theme) {
        ...
        self.breadcrumb.layout(Rect::new(
        ...
        self.new_folder_button.layout(Rect::new(
        ...
        self.file_list.layout(Rect::new(
        ...
        if self.mode == FileDialogMode::Save {
            self.filename_input.layout(Rect::new(
        ...
        // Filter dropdown
        self.filter_dropdown.layout(Rect::new(
            self.footer_rect.x() + padding + (if self.mode == FileDialogMode::Save { input_width + 10 } else { 0 }),
            self.footer_rect.y() + (footer_height - 30) / 2,
            filter_width,
            30
        ), theme);
        ...
        // OK button
        self.ok_button.layout(Rect::new(
    ...
    fn handle_event(&mut self, event: &Event, theme: &Theme) -> EventResult {
        ...
        if let Some(item) = self.file_list.selected_item() {
            let selected_index = self.entries.iter().position(|e| e.path.to_string_lossy() == item.id).unwrap_or(0);
