use crate::{Container, Label, ListItem, ListView, TextInput, TreeNode, TreeView};
use erigui_core::{
    DrawContext, Event, EventResult, LayoutConfig, LayoutConstraints, LayoutMode, Margins, Rect,
...
pub struct FileManager {
    ...
    _path_input_id: WidgetId,
    _tree_view_id: WidgetId,
    _list_view_id: WidgetId,
...
impl FileManager {
    pub fn new(
        ...
        Self {
            ...
            container_id,
            _path_input_id: path_input_id,
            _tree_view_id: tree_view_id,
            _list_view_id: list_view_id,
            _status_label_id: status_label_id,
...
pub fn create_file_manager(widget_manager: &mut crate::WidgetManager) -> WidgetId {
    // Create container
    let container =
        Container::new(widget_manager.add_widget(Box::new(Container::new(WidgetId::default()))))
    ...
    // Create path input
    let path_input =
        TextInput::new(widget_manager.add_widget(Box::new(TextInput::new(WidgetId::default()))))
    ...
    // Create tree view
    let tree_view =
        TreeView::new(widget_manager.add_widget(Box::new(TreeView::new(WidgetId::default()))));
    let tree_view_id = tree_view.id();

    // Create list view
    let list_view =
        ListView::new(widget_manager.add_widget(Box::new(ListView::new(WidgetId::default()))));
    ...
    widget_manager
        ...
        .unwrap()
        .add_child(path_input_id);
    widget_manager
        .get_typed_mut::<Container>(container_id)
        .unwrap()
        .add_flex_child(split_container_id, 1.0);
    widget_manager
        .get_typed_mut::<Container>(container_id)
        .unwrap()
        .add_child(status_label_id);

    widget_manager
        .get_typed_mut::<Container>(split_container_id)
        .unwrap()
        .add_flex_child(tree_view_id, 0.3);
    widget_manager
        .get_typed_mut::<Container>(split_container_id)
        .unwrap()
        .add_flex_child(list_view_id, 0.7);
