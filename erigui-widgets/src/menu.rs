...
impl MenuBar {
    ...
    pub fn draw_menu_bar_only(&self, context: &mut dyn DrawContext, theme: &Theme) {
        ...
        for (i, menu) in self.menu_items.iter().enumerate() {
            ...
            if is_active {
                ...
                println!("Drawing menu bar hover for item {} at rect {:?}", i, menu_rect);
            ...
            let text_y = menu_rect.y() + (menu_rect.height() + theme.typography.font_size_base) / 2 - 2;
            context.draw_text(
                &menu.text,
    ...
    fn get_menu_rect(&self, menu_index: i32) -> Rect {
        ...
        Rect::new(x, self.state.bounds.y(), self.item_widths[menu_index as usize], self.state.bounds.height())
    }
    ...
    fn dropdown_item_from_point(&self, point: Point, menu_index: i32) -> i32 {
        ...
        println!("Dropdown item check: point {:?}, dropdown rect {:?}, relative_y {}, item_index {}", 
                 point, dropdown_rect, relative_y, item_index);
        
        if item_index >= 0 && item_index < self.menu_items[menu_index as usize].submenu_items.len() as i32 {
            item_index
        } else {
            -1
        }
    ...
    fn draw_dropdown(&self, context: &mut dyn DrawContext, theme: &Theme, menu_index: i32) {
        ...
        for (i, item) in items.iter().enumerate() {
            ...
            let item_rect = Rect::new(dropdown_rect.x(), item_y, dropdown_rect.width(), self.dropdown_height);
            let is_hover = i as i32 == self.dropdown_hover;
            
            // Draw item background
            if is_hover && item.enabled {
                println!("Drawing hover for item {} in dropdown at rect {:?}", i, item_rect);
            ...
            if item.is_separator {
                ...
                let text_y = item_y + (self.dropdown_height + theme.typography.font_size_base) / 2 - 2;
                context.draw_text(
                    &item.text,
...
impl Widget for MenuBar {
    ...
    fn measure(&self, _constraints: &LayoutConstraints, theme: &Theme) -> Size {
        ...
        Size::new(total_width, theme.typography.font_size_base + 2 * self.padding)
    }
    ...
    fn handle_event(&mut self, event: &Event, _theme: &Theme) -> EventResult {
        ...
        match event {
            Event::MouseButton(mouse_event) => {
                ...
                if mouse_event.button == MouseButton::Left && mouse_event.pressed {
                    ...
                    if self.dropdown_visible && self.active_menu >= 0 {
                        ...
                        if dropdown_item >= 0 {
                            let item = &self.menu_items[self.active_menu as usize].submenu_items[dropdown_item as usize];
            ...
            Event::MouseMove(mouse_event) => {
                ...
                if self.state.bounds.contains(point) || (self.dropdown_visible && self.active_menu >= 0) {
