use erigui_core::{
    ...
    Point, Rect, Size, Theme, Widget, WidgetId, WidgetState, Color,
};
use std::any::Any;
        ...
        for (index, item) in self.notifications.iter_mut().enumerate() {
            ...
            let (x, y) = match self.position {
                ...
                NotificationPosition::TopRight => (
                    viewport.width() - self.notification_width - self.margin - item.animation_offset,
                    self.margin + y_offset
                ),
                NotificationPosition::BottomLeft => (
                    self.margin + item.animation_offset,
                    viewport.height() - self.margin - (self.notification_height + self.spacing) * (index + 1) as i32 + self.spacing
                ),
    ...
    pub fn update_animations(&mut self, delta_time: f32) {
        ...
        for item in &mut self.notifications {
            if item.animation_offset > 0 {
                item.animation_offset = (item.animation_offset as f32 - self.animation_speed * delta_time).max(0.0) as i32;
            }
        }
...
impl Widget for NotificationManager {
    ...
    fn draw(&self, context: &mut dyn DrawContext, theme: &Theme) {
        ...
        for (index, item) in self.notifications.iter().enumerate() {
            ...
            let icon_name = item.notification.icon.as_deref()
            ...
            // Draw action button
            if let (Some(action_rect), Some(action_text)) = (&item.action_button_rect, &item.notification.action_text) {
                ...
                context.draw_text(
                    action_text,
                    Point::new(
                        action_rect.center().x - 20,
                        action_rect.center().y + 4
                    ),
                    theme.typography.font_size_small
                );
            }
