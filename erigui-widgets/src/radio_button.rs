...
impl RadioButton {
    pub fn new(id: WidgetId, text: impl Into<String>, group_id: u32) -> Self {
        ...
        RadioGroupManager::instance().lock().unwrap().register_button(id, group_id);
    ...
    pub fn with_checked(mut self, checked: bool) -> Self {
        ...
        if checked {
            RadioGroupManager::instance().lock().unwrap().set_selected(self.group_id, self.state.id);
        }
        self
    }
    ...
    fn draw(&self, context: &mut dyn DrawContext, theme: &Theme) {
        ...
        if !self.text.is_empty() {
            ...
            let (text_x, text_y) = match self.label_position {
                ...
                LabelPosition::Top => (
                    self.state.bounds.x() + (self.state.bounds.width() - self.text.len() as i32 * theme.typography.font_size_base * 3 / 5) / 2,
            ...
            context.draw_text(&self.text, Point::new(text_x, text_y), theme.typography.font_size_base);
        }
    }
    ...
    fn handle_event(&mut self, event: &Event, _theme: &Theme) -> EventResult {
        ...
        match event {
            ...
            Event::Update => {
                ...
                let selected_id = RadioGroupManager::instance().lock().unwrap().get_selected(self.group_id);
