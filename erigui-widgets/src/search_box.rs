...
impl SearchBox {
    ...
    pub fn with_suggestion_provider<F: FnMut(&str) -> Vec<String> + 'static>(mut self, f: F) -> Self {
    ...
    fn trigger_change(&mut self) {
        ...
        if text.is_empty() {
            ...
            self.suggestions = self.search_history
                .iter()
        ...
        match event {
            Event::MouseButton(MouseButtonEvent {
                ...
                if self.state.bounds.contains(*position) {
                    ...
                    // Check clear button
                    if self.clear_button_rect.contains(*position) && !self.input.get_text().is_empty() {
                        self.clear();
            ...
            Event::MouseMove(move_event) => {
                ...
                if self.show_suggestions && self.suggestions_rect.contains(move_event.position) {
                    let item_height = theme.typography.font_size_base + 8;
                    let index = ((move_event.position.y - self.suggestions_rect.y()) / item_height) as usize;
                    if index < self.suggestions.len() {
            ...
            Event::KeyPress(KeyPressEvent { key, modifiers: _modifiers, .. }) => {
