use erigui_core::{
    ...
    Point, Rect, Size, Theme, Widget, WidgetId, WidgetState, Key, KeyPressEvent, MouseMoveEvent,
    Modifiers,
};
use std::any::Any;
...
impl TextArea {
    ...
    fn insert_char(&mut self, ch: char) {
        ...
        if ch == '\n' {
            ...
            self.lines[self.cursor_line].insert_str(self.cursor_col, &spaces);
    ...
    fn delete_char_forward(&mut self) {
        if self.selection.is_some() {
            ...
            self.delete_selection();
            self.notify_change();
        } else if self.cursor_col < self.lines[self.cursor_line].len() {
            self.save_state();
            self.lines[self.cursor_line].remove(self.cursor_col);
            self.notify_change();
        ...
        if let Some(sel) = &self.selection {
            if !sel.is_empty() {
                ...
                if start_line == end_line {
                    ...
                    let remainder = self.lines[end_line][end_col..].to_string();
    ...
    fn ensure_cursor_visible(&mut self) {
        ...
        let content_x = if self.line_numbers { self.gutter_width } else { 0 };
        ...
        } else if cursor_y + self.line_height > self.scroll_offset.y + self.state.bounds.height() - 20 {
...
impl Widget for TextArea {
    ...
    fn draw(&self, context: &mut dyn DrawContext, theme: &Theme) {
        ...
        let content_x = self.state.bounds.x() + if self.line_numbers { self.gutter_width } else { 0 };
        let content_rect = Rect::new(
            content_x,
            self.state.bounds.y(),
            self.state.bounds.width() - if self.line_numbers { self.gutter_width } else { 0 },
        ...
        if let Some(sel) = &self.selection {
            if !sel.is_empty() {
                ...
                if start_line == end_line {
                    ...
                    let y = self.state.bounds.y() + start_line as i32 * self.line_height - self.scroll_offset.y;
                    ...
                    let x = content_x + start_col as i32 * self.char_width - self.scroll_offset.x;
                    let y = self.state.bounds.y() + start_line as i32 * self.line_height - self.scroll_offset.y;
                    let width = (self.lines[start_line].len() - start_col) as i32 * self.char_width;
                    context.fill_rect(Rect::new(x, y, width, self.line_height));
                    
                    // Middle lines
                    for line in (start_line + 1)..end_line {
                        let y = self.state.bounds.y() + line as i32 * self.line_height - self.scroll_offset.y;
                        let width = self.lines[line].len() as i32 * self.char_width;
                        context.fill_rect(Rect::new(content_x - self.scroll_offset.x, y, width, self.line_height));
                    }
                    
                    // Last line
                    if end_line < self.lines.len() {
                        let y = self.state.bounds.y() + end_line as i32 * self.line_height - self.scroll_offset.y;
        ...
        if self.state.focused {
            context.set_color(theme.colors.text);
            let cursor_x = content_x + self.cursor_col as i32 * self.char_width - self.scroll_offset.x;
            let cursor_y = self.state.bounds.y() + self.cursor_line as i32 * self.line_height - self.scroll_offset.y;
        ...
        match event {
                ...
                match key {
                    Key::Left => {
                        if self.cursor_col > 0 {
                            self.move_cursor(self.cursor_line, self.cursor_col - 1, modifiers.contains(Modifiers::SHIFT));
                        } else if self.cursor_line > 0 {
                            self.move_cursor(self.cursor_line - 1, self.lines[self.cursor_line - 1].len(), modifiers.contains(Modifiers::SHIFT));
                        }
                        return EventResult::Consumed;
                    }
                    Key::Right => {
                        if self.cursor_col < self.lines[self.cursor_line].len() {
                            ...
                            self.move_cursor(self.cursor_line + 1, 0, modifiers.contains(Modifiers::SHIFT));
                        }
                        return EventResult::Consumed;
                    }
                    Key::Up => {
                        if self.cursor_line > 0 {
                            self.move_cursor(self.cursor_line - 1, self.cursor_col, modifiers.contains(Modifiers::SHIFT));
                        }
                        return EventResult::Consumed;
                    }
                    Key::Down => {
                    ...
                    Key::Home => {
                        if modifiers.contains(Modifiers::CTRL) {
                            ...
                            self.move_cursor(self.cursor_line, 0, modifiers.contains(Modifiers::SHIFT));
                        }
                        return EventResult::Consumed;
                    }
                    Key::End => {
                        if modifiers.contains(Modifiers::CTRL) {
                            ...
                            self.move_cursor(last_line, self.lines[last_line].len(), modifiers.contains(Modifiers::SHIFT));
                        } else {
                            self.move_cursor(self.cursor_line, self.lines[self.cursor_line].len(), modifiers.contains(Modifiers::SHIFT));
                        }
                        return EventResult::Consumed;
                    }
                    Key::Backspace => {
                            ...
                            match ch {
                                ...
                                'a' => {
                                    self.selection = Some(Selection {
                                        ...
                                        end_col: self.lines[self.lines.len() - 1].len(),
                                    });
                                }
                                _ => {}
                            }
            ...
            Event::MouseWheel(wheel_event) => {
                if self.state.bounds.contains(wheel_event.position) {
                    self.scroll_offset.y = (self.scroll_offset.y - wheel_event.delta.y * 3)
                        .max(0)
                        .min((self.lines.len() as i32 * self.line_height).saturating_sub(self.state.bounds.height()));
                    
                    if wheel_event.modifiers.contains(Modifiers::SHIFT) {
                        self.scroll_offset.x = (self.scroll_offset.x - wheel_event.delta.x * 3)
                            .max(0);
                    }
