...
impl TreeView {
    ...
    fn draw_node(
        ...
        context.draw_text(&node.text, Point::new(text_x, text_y), theme.typography.font_size_base);
...
impl Widget for TreeView {
    ...
    fn draw(&self, context: &mut dyn DrawContext, theme: &Theme) {
        ...
        if total_height > self.state.bounds.height() {
            ...
            // Scrollbar thumb
            let thumb_height = (self.state.bounds.height() as f32 * self.state.bounds.height() as f32 / total_height as f32) as i32;
            let thumb_y = self.state.bounds.y() + (self.scroll_offset as f32 * self.state.bounds.height() as f32 / total_height as f32) as i32;
        ...
        match event {
            Event::MouseButton(MouseButtonEvent {
                ...
                if let Some(node_id) = self.node_at_position(*position) {
                    ...
                    fn find_node_info(nodes: &[TreeNode], target_id: &str, current_y: &mut i32, current_depth: i32, item_height: i32) -> Option<i32> {
                        for node in nodes {
                            ...
                            *current_y += item_height;
                            ...
                            if node.expanded {
                                if let Some(depth) = find_node_info(&node.children, target_id, current_y, current_depth + 1, item_height) {
                                    return Some(depth);
                                }
                            }
                        }
                    ...
                    if let Some(depth) = find_node_info(&self.root_nodes, &node_id, &mut node_y, 0, self.item_height) {
                        node_depth = depth;
                    }
                    ...
                    if let Some(node) = self.find_node_mut(&node_id) {
                        if !node.children.is_empty() && click_x >= arrow_x && click_x <= arrow_x + 16 {
                            clicked_expand = true;
                        }
                    }
                    ...
                    // Handle the click
                    let (expanded_changed, new_expanded) = if let Some(node) = self.find_node_mut(&node_id) {
                        node.selected = true;
